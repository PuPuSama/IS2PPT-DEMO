...
_PYRAMID_METHODOLOGY = """\
- Conclusion first: lead each section with its core point.
- Top-down: each higher-level point summarizes everything beneath it.
- MECE grouping: items at the same level share one logical category, mutually exclusive and collectively exhaustive.
- Logical progression: order content by a clear logic (time, importance, cause-effect, etc.)."""


def _format_research_context(research_context: Optional[str]) -> str:
    """Inject the web-research briefing so generated content reflects real facts."""
    ...
    return (
        ...
        "</research_context>\n"
        "Note: Ground the generated content in the research context above (current facts and market reality). "
        "Do NOT fabricate; if a technology shown above is outdated, do not present it as a core recommendation.\n\n"
    )


def get_outline_generation_prompt(project_context: 'ProjectContext', language: str = None) -> str:
    """生成 PPT 大纲的 prompt（JSON 输出，金字塔原理 + 可选联网调研上下文）"""
    idea_prompt = project_context.idea_prompt or ""
    research_block = _format_research_context(getattr(project_context, 'research_context', None))

    prompt = (f"""\
You are a top-tier PPT structure architect. Build a logically rigorous, clearly layered outline using the Pyramid Principle:
{_PYRAMID_METHODOLOGY}

{research_block}You can organize the content in two ways:
...
def get_search_queries_prompt(topic: str, max_queries: int = 4, language: str = None) -> str:
    """Turn a topic into a few diverse web-search queries (one per line)."""
    return f"""You are a research assistant preparing to build a presentation.
Generate up to {max_queries} concise, diverse web search queries that gather the most useful, up-to-date facts, data, market status, key players, and trends for the topic.
Rules:
- One query per line. No numbering, no quotes, no extra commentary.
- Vary the angle (overview, latest data/statistics, key players/products, trends/risks).
...
def get_research_summary_prompt(topic: str, results_text: str, language: str = None) -> str:
    """Distill raw search results into a concise factual briefing for outlining."""
    return f"""You are a research analyst. Below are raw web search results for a presentation topic.
Distill them into a concise, factual briefing the slide author can rely on.
Rules:
- Focus on concrete facts, figures, dates, named entities, current status and trends.
- Use short bullet points grouped logically. Keep it under ~250 words.
- Do NOT invent facts not present in the results. If sources look outdated or conflict, note it briefly.
...
严禁编造、猜测或改写图片路径。只能逐字引用上下文中已经真实出现的图片 URL。不要输出类似 /files/队标.png、/files/图片.jpg 这种未提供的路径；如果没有真实图片 URL，请只在“视觉元素”等字段中描述需要的画面。
{get_language_instruction(language)}
""")
...
# SVG 模式：施工图里只有“一句话主旨/要点与数据”是会渲染到页面上的正文文字，其余规划性
# 元信息全部抽成额外字段（前端描述编辑器正文只剩可见文字，规划信息收纳到字段块）。
# 注意：解析器(_parse_extra_fields)把正文取到“第一个字段出现之前”，所以 prompt 必须把
# 正文(主旨+要点)排在最前、这些字段排在后面，详见 get_svg_page_description_prompt。
SVG_DESC_EXTRA_FIELDS = ['页面类型', '便当版面组合', '视觉层级', '占位元素', '演讲者备注']

# SVG 设计阶段(get_svg_generation_prompt)仍需要的“版面规划”字段：生成 SVG 前把它们拼回正文，
# 当作排版施工指导。演讲者备注是配音用、不渲染，不拼回。
SVG_LAYOUT_PLAN_FIELDS = ['页面类型', '便当版面组合', '视觉层级', '占位元素']

# SVG 施工图阶段的“信息密度预算”（一页 16:9 的容量上限，按详细度分档）。
# 文字爆版的根因是内容量在这一步就失控，SVG 设计阶段再怎么控宽控行也救不回来，
# 所以从源头按档位卡死：每页要点数、单条字数、整页正文总字数。
SVG_DESC_BUDGET = {
    'concise':  {'max_points': 3, 'per_point_chars': 12, 'total_chars': 90},
    'default':  {'max_points': 4, 'per_point_chars': 20, 'total_chars': 150},
    'detailed': {'max_points': 5, 'per_point_chars': 32, 'total_chars': 240},
}


def _svg_plan_instructions(detail_level: str = 'default') -> str:
    """SVG 施工图的公共指令体：信息密度预算 + 便当网格方法论 + 每页字段定义。

    逐页版(get_svg_page_description_prompt)与流式版(get_svg_all_descriptions_stream_prompt)
    共用此块——字段定义/预算只此一处，避免两份 SVG prompt 漂移(改一处即两边同步)。
    """
    budget = SVG_DESC_BUDGET.get(detail_level, SVG_DESC_BUDGET['default'])
    return f"""\
## 信息密度预算（硬约束，必须遵守——一页 16:9 放不下太多字，超量必然排版错乱）
- 文字风格：{DETAIL_LEVEL_SPECS[detail_level]}
- 每页要点 **最多 {budget['max_points']} 条**；每条 **不超过 {budget['per_point_chars']} 个字**（用短语/数据，不写整句）。
- 每页可渲染正文（标题除外）总量 **控制在约 {budget['total_chars']} 字以内**。
- 宁可少而精：信息超量时**主动取舍、合并或拆到下一页**，绝不硬塞；该留白就留白。

## 便当网格（Bento Grid）版面方法论
版面由内容需求驱动、而非套固定模板。用不同尺寸卡片组合，最重要的信息放最大的卡片；卡片间距 ≥20px。
常用组合：单一焦点／两栏(50-50 或 2:1 非对称)／三栏并列／主次结合(中间大卡+两侧小卡)／顶部英雄卡+下方小卡网格／自由混合网格。

## 每页字段（严格按下列顺序逐行输出；不要写颜色/字体/动效；不要 markdown 代码围栏）
**前两项是会渲染到页面上的正文文字，必须排在最前；其余为规划信息，必须排在它们之后。**
一句话主旨：这页要让听众记住的唯一核心信息（会作为页面主文字呈现）
要点与数据：本页承载的要点/关键数据，**最多 {budget['max_points']} 条、每条 ≤{budget['per_point_chars']} 字**（取自大纲与素材，缺数据不要编造；会渲染到页面）
页面类型：封面/目录/章节过渡/内容页/结尾页（取其一）
便当版面组合：从上面方法论里选一种最契合的，并一句话说明为什么最适合本页信息
视觉层级：哪一块卡片最大/最重要，其余如何主次排布
占位元素：是否需要图表/图标/配图，用文字占位标注，如 [图表：近三年增长曲线]、[图标：芯片]；无则写“无”
演讲者备注：一句话讲稿，引出/串联本页（用于配音，不渲染到页面）"""


def get_svg_page_description_prompt(project_context: 'ProjectContext', outline: list,
                                    page_outline: dict, page_index: int,
                                    part_info: str = "",
                                    language: str = None,
                                    detail_level: str = "default",
                                    extra_fields: list = None) -> str:
    """生成单页“SVG 施工图”描述（SVG 生成模式的页面描述）。

    与图片路线的 get_page_description_prompt 并列；改装自参考项目 joker-sxj/ppt-agent 的
    prompts/03-planning.md（策划稿·逐页版面规划，MIT）。**只规划结构、不写视觉效果**
    （颜色/动效/材质留给 SVG 设计阶段），产出作为 get_svg_generation_prompt 的输入“施工图”。
    """
    original_input = _get_original_input(project_context)
    research_block = _format_research_context(getattr(project_context, 'research_context', None))
    cover_note = (
        "\n**本页是封面页**：保持极简——只规划主标题/副标题/演讲人，不堆要点与图表占位。"
        if page_index == 1 else ""
    )

    prompt = (f"""\
你是顶尖 PPT 公司的**策划师**：设计师只管样式，版面骨架由你定死——每页放什么、怎么排、谁主谁次。
**本阶段只规划结构，不写视觉效果（颜色/字体/动效/材质留给后续 SVG 设计阶段）。**

用户的原始需求：\n{original_input}\n
{research_block}
完整大纲：\n{outline}\n{part_info}
{_format_requirements(project_context.description_requirements, "description")}现在为第 {page_index} 页输出“版面施工图”：
{page_outline}{cover_note}

{_svg_plan_instructions(detail_level)}
{get_language_instruction(language)}
""")

    return _build_prompt(prompt, project_context.reference_files_content, tag='get_svg_page_description_prompt')


def get_svg_all_descriptions_stream_prompt(project_context: 'ProjectContext',
                                           outline: list,
                                           flat_pages: list,
                                           language: str = None,
                                           detail_level: str = "default",
                                           extra_fields: list = None) -> str:
    """一次性生成所有页“SVG 施工图”描述（SVG 流式生成）。

    与逐页版 get_svg_page_description_prompt 共用 _svg_plan_instructions(字段/预算只一处)，
    这里只额外套上流式分页协议(BEGIN/PAGE_END/END)。
    """
    original_input = _get_original_input(project_context)
    research_block = _format_research_context(getattr(project_context, 'research_context', None))
    ...
    prompt = (f"""\
你是顶尖 PPT 公司的**策划师**：为下面整套 PPT 逐页输出“版面施工图”。设计师只管样式，版面骨架由你定死。
**本阶段只规划结构，不写视觉效果（颜色/字体/动效/材质留给后续 SVG 设计阶段）。**
...
{_format_requirements(project_context.description_requirements, "description")}请按顺序为每一页输出施工图：先输出 `<!-- BEGIN -->`，然后逐页输出，每页以 `<!-- PAGE_END -->` 结束，全部完成后输出 `<!-- END -->`。
**第一页若为封面：保持极简——只规划主标题/副标题/演讲人，不堆要点与图表占位。**

{_svg_plan_instructions(detail_level)}

## 整体输出格式（每页严格用上面的字段顺序，逐页用 PAGE_END 分隔）
```
<!-- BEGIN -->
一句话主旨：……
要点与数据：……
...
""")
    ...
    return _build_prompt(prompt, project_context.reference_files_content, tag='get_svg_all_descriptions_stream_prompt')
...
提示：如果参考文件中包含以 /files/ 开头的本地文件URL图片（例如 /files/mineru/xxx/image.png），请将这些图片以markdown格式输出，例如：![图片描述](/files/mineru/xxx/image.png)，而不是作为普通文本。严禁编造、猜测或改写图片路径；只能逐字引用上下文中真实出现过的图片 URL。如果没有真实图片 URL，请用文字描述素材需求，不要写 markdown 图片链接。
...
# 字体白名单：必须与 backend/fonts/ 内置字体的“内部 family 名”一致。
# NotoSansSC-Regular.ttf 注册名是 "Noto Sans CJK SC"（不是 "Noto Sans SC"）。
# 同一个免费字体贯穿全链路：SVG 声明它（浏览器预览）、resvg 渲染 PNG、并被嵌入导出的
# PPTX，所以三端/各平台字体一致。需与 svg_contract.SVG_FONT_FAMILY 保持一致。
SVG_FONT_FAMILY = "Noto Sans CJK SC"


def get_svg_generation_prompt(page_desc: str, outline_text: str,
                              current_section: str,
                              has_material_images: bool = False,
                              extra_requirements: str = None,
                              language: str = None,
                              ...
                              page_index: int = 1,
                              aspect_ratio: str = "16:9") -> str:
    """生成“整页 SVG”设计 prompt（SVG 生成模式）。

    与 get_image_generation_prompt 同参数，便于在 task_manager 中按 generation_mode
    无缝切换。产出受严格契约约束的纯 SVG（见 docs/PRD-svg-slide-generation.md §4），
    以便渲染（resvg）与后续 SVG→可编辑 PPTX 导出。

    思路改装自参考项目 joker-sxj/ppt-agent（prompts/04-design-svg.md + 便当网格方法论，
    MIT）；关键差异：字体锁死为内置白名单 SVG_FONT_FAMILY，禁系统字体栈。
    """
    material_images_note = ""
    if has_material_images:
        material_images_note = (
            "\n- 已额外提供素材图片：可用 `<image href=\"...\">` 引用其 URL/项目内 /files 路径嵌入，"
            "按页面需要智能取用，不要强行塞入。"
        )
    ...
    template_style_guideline = (
        "- 配色与设计语言参照模板风格，但**只借鉴风格、禁止照搬模板里的任何文字**。"
        if has_template else
        "- 严格按风格描述设计；配色克制、留白充分、对齐严谨。"
    )

    cover_note = (
        "\n- 本页是**封面页**：用专业封面美学突出标题，主次分明，第一眼抓住注意力。"
        if page_index == 1 else ""
    )

    prompt = (f"""\
你是精通信息架构与 SVG 编码的专家。把下面这一页的内容，转化为一张高质量、结构化、
有高级感与专业感的**整页 SVG 幻灯片**。

当前页所属章节：{current_section}
<page_description>
{page_desc}
</page_description>

## 硬性要求（违反则不可用）
1. **画布**：根元素必须是 `<svg viewBox="0 0 1280 720" xmlns="http://www.w3.org/2000/svg">`；
   所有元素不得溢出画布（{aspect_ratio} 比例，逻辑画布固定 1280×720）。
2. **文字（可编辑性关键）**：
   - 一律用 `<text>`/`<tspan>` 承载，**不要转曲、不要转 path**，每个都显式写 `x`/`y`/`font-size`/`fill`。
   - `font-family` **只能且必须**是 `"{SVG_FONT_FAMILY}"`（精确到这个名字，含大小写空格）。写别的字体名会渲染成空白方块。
   - 长文本自行折成多个 `<tspan>` 手动换行，**不要依赖自动换行**。
3. **不引用任何外部资源**（图片/字体/CSS 的 URL、`@import`、外链 `<style>` 一律禁止）；
   也禁止 `<foreignObject>`、`<script>`、事件属性（on*）。
4. **图标**：用内联矢量 `<path>` 绘制（带 `fill`，天然透明），不要用图标字体或外链图标。
   **统一画法**：图标放进 `<g data-role="icon" transform="translate(cx,cy)">`，内部 `<path>` 以原点为中心绘制（坐标落在约 −12..12），这样只需给出容器中心 `cx,cy` 即可精确居中，避免逐点估算导致偏移。
5. **形状**：`<rect>/<circle>/<ellipse>/<line>/<polyline>/<polygon>/<path>`，可用 `fill/stroke/opacity/rx`。
6. **渐变**允许（`linearGradient`/`radialGradient`）；**复杂滤镜（模糊/阴影）尽量不用**（渲染支持有限、且不利于转 PPTX）。
7. **语义分组**：每个内容块用 `<g data-role="...">` 包裹，`data-role` 取值
   ∈ {{title, subtitle, body, bullet, icon, image, shape, footer, page-number, decoration}}；
   文本块可加 `data-text` 原文。（便于后续映射成 PPTX 原生对象，请认真打标。）
8. **自包含**：整页就是一个 `<svg>…</svg>` 字符串，无任何外部依赖。
9. **良构 XML**：必须是严格良构的 XML——同一元素上**同名属性只能出现一次**（如 `opacity`、`fill` 不可重复），所有标签正确闭合、属性值都带引号。

## 版面方法论（便当网格 Bento Grid）
版面由**内容需求驱动，而非套固定模板**。用不同尺寸的卡片组合出动态而有序的布局：
- **层级**：最重要的信息放最大的卡片；用卡片尺寸建立视觉层级。
- **留白**：卡片之间至少 20px 间距。
- **常用组合**（按 1280×720 换算）：单一焦点（一张约 1200×580 大卡）／两栏 50-50 或 2:1 非对称／
  三栏等宽并列／主次结合（中间大卡+两侧小卡）／顶部英雄卡+下方 2-4 小卡网格／自由混合网格。
按本页内容选最合适的一种，不要硬凑。

## 版面纪律（防偏移 / 防溢出，必须遵守）
**坐标要算出来，不要凭感觉估**——以下规则让居中与包含变成机械计算：
1. **安全区**：所有可见内容必须落在 `x∈[40,1240]`、`y∈[40,680]`（页边距 40px，可用区 1200×640）；任何元素都不得越出该范围，更不得越出画布。
2. **先定卡片骨架**：先用 `<rect data-role="shape">` 把每张卡片的 `x/y/width/height`（含 `rx`）写定，再往里填内容；卡片之间间距 ≥20px。
3. **容器包含律（解决“小卡超出大卡”）**：放进某张卡片的所有子元素（小卡 / 文字 / 图标）必须完整落在该卡内部，留 ≥24px 内边距：
   - 水平：`子.x ≥ 卡.x + 24` 且 `子.x + 子.w ≤ 卡.x + 卡.w − 24`
   - 垂直：`子.y ≥ 卡.y + 24` 且 `子.y + 子.h ≤ 卡.y + 卡.h − 24`
   - 嵌套小卡同样要满足；**放不下就减少数量或缩小尺寸，绝不允许溢出父卡**。
4. **居中要算、不要估**：容器中心 `cx = 卡.x + 卡.w/2`、`cy = 卡.y + 卡.h/2`。
   - 居中文字：`text-anchor="middle"`，`x` 取 `cx`；需垂直居中再加 `dominant-baseline="middle"`（或显式给 baseline 的 y）。
   - 图标按上面“统一画法”用 `translate(cx,cy)` 居中。
5. **对齐网格**：同一行的卡片共享同一 `y` 与 `height`；同一列共享同一 `x` 与 `width`；左对齐的标题/正文共享同一 `x` 基线。
6. **文字不溢出（手动控宽控行）**：`<text>` 的渲染宽度由字数决定，必须自己估算并手动折行，不要依赖自动换行——
   - 单字宽估算（`Noto Sans CJK SC`）：中文/全角标点 ≈ `font-size` px/字；英文、数字、半角 ≈ `0.6 × font-size` px/字。
   - **每行字数上限** ≈ `(卡.w − 48) / 单字宽`；超过就拆成多个 `<tspan>`，每个 `<tspan>` 显式给 `x`（左对齐取卡左内边距、居中取 `cx`）和递增的 `y`，行距 ≈ `1.4 × font-size`。
   - **竖向**：`行数 × 行距 ≤ 卡.h − 48`；放不下就**调小 `font-size` 或精简文字**（为演示服务，宁可少字也不要溢出），不要硬塞。
   - 标题、长串数字、英文术语最容易超宽，务必按上面公式预算后再定 `font-size`。
7. **输出前自检**：① 有无元素越出安全区/画布；② 每张小卡是否完整在父卡内（满足第 3 条不等式）；③ 居中元素的 `x` 是否等于其容器中心 `cx`；④ 每行文字按字宽估算是否超出卡片内宽、总行高是否超出卡片内高。发现越界就调整坐标/字号/字数后再输出。

## 风格
{template_style_guideline}{cover_note}{material_images_note}{extra_req_text}
{get_ppt_language_instruction(language)}

## 输出格式
**只输出 SVG 代码本身**，从 `<svg` 到 `</svg>` 为止；不要任何解释文字，不要 markdown 代码围栏（```）。
""")

    logger.debug(f"[get_svg_generation_prompt] Final prompt:\n{prompt}")
    return prompt
