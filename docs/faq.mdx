...
Editable PPTX export is only available for SVG-route projects. If an older image-route project fails, regenerate the slides with the SVG route and export again.

For SVG-route projects, enable **Allow Partial Results** in **Project Settings → Export Settings** if you need a best-effort file while investigating individual conversion failures.
...
- Docker users who need PPTX support inside the container:
  ```bash
  docker exec -it is2ppt-backend bash -c "apt-get update && apt-get install -y libreoffice-impress"
  ```
