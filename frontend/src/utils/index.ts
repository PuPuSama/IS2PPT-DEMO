...
export function normalizeErrorMessage(errorMessage: string | null | undefined): string {
  ...
  if (
    message.includes('no template image found')
    || message.includes('no template image or style description')
    || message.includes('请先上传模板图片或添加风格描述')
  ) {
    return isZh
      ? '当前项目还没有模板图片或风格描述，请先点击页面工具栏的"更换模板"按钮，选择/上传模板图片，或填写一段文字风格描述后再生成。'
      : 'No template image or style description found. Please select/upload a template image, or enter a text style description first.';
