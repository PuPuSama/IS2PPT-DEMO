...
class ExportError(Exception):
    ...
    def _get_default_help_text(self, error_type: str) -> str:
        """根据错误类型返回默认帮助提示"""
        help_texts = {
            'style_extraction': '旧图片路线的样式提取已下线。请使用 SVG 生成路线重新生成页面；如需尽力导出，可在「项目设置 -> 导出设置」中开启「返回半成品」。',
            'text_render': '文本渲染失败可能是由于字体或编码问题。请检查页面内容是否包含特殊字符。',
            'image_add': '图片添加失败可能是由于图片文件损坏或路径错误。请尝试重新生成该页面的图片。',
            'inpaint': '背景修复失败可能是由于图像修复 API 配置问题。请检查全局设置中的相关服务配置。',
            'config': '配置错误。请检查项目设置或全局设置中的相关配置。',
