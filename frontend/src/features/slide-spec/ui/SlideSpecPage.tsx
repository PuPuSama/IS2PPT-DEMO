...
import { ArrowLeft, ArrowRight, FileText, Sparkles, Download, Upload, ChevronDown, Settings2, X, Plus, HelpCircle, ImageIcon, Presentation } from 'lucide-react';
...
import {
  exportProjectToMarkdown as exportLegacyDeckToMarkdown,
  parseMarkdownPages as parseLegacyMarkdownSlides,
} from '@/utils/projectUtils';
import {
  appendSlideSpecs,
  clearSourceDeckJob,
  defaultImagePromptFields,
  defaultSlideSpecFields,
  DEFAULT_SLIDE_SPEC_FIELDS,
  loadSlideSpecPreferences,
  loadSourceDeckJob,
  readSlideSpecFieldCatalog,
  readSourceDeckJobId,
  refineSlideSpecs,
  saveSlideSpecFieldCatalog,
  saveSlideSpecPreferences,
  saveSlideSpecRequirements,
  type SlideSpecPreferencesPatch,
} from '../api/slideSpecRepository';
import {
  markSlideSpecGenerating,
  slideHasSpec,
  slideIdentity,
  slideSpecSnapshotFromProject,
} from '../model/slideSpecSnapshot';
...
export const SlideSpecPage: React.FC = () => {
  ...
  const {
    ...
    generateDescriptions: generateSlideSpecs,
    generatePageDescription: generateSingleSlideSpec,
    regenerateRenovationPage: regenerateSourceSlide,
  } = useProjectStore();
  const { show, ToastContainer } = useToast();
  const { confirm, ConfirmDialog } = useConfirm();
  const [isAiRefining, setIsAiRefining] = React.useState(false);
  const [previewFileId, setPreviewFileId] = useState<string | null>(null);
  const [isSourceDeckProcessing, setIsSourceDeckProcessing] = useState(false);
  const [sourceDeckProgress, setSourceDeckProgress] = useState<{ total: number; completed: number } | null>(null);
  ...
  const [activeFieldNames, setActiveFieldNames] = useState<string[]>(() => defaultSlideSpecFields());
  const [imagePromptFields, setImagePromptFields] = useState<string[]>(() => defaultImagePromptFields());
  const [availableFields, setAvailableFields] = useState<string[]>(() => readSlideSpecFieldCatalog());
  ...
  const slideSpec = useMemo(
    () => slideSpecSnapshotFromProject(deckSnapshot),
    [deckSnapshot],
  );
  const slides = slideSpec?.slides ?? EMPTY_SLIDES;

  useEffect(() => {
    if (!slideSpec) return;
    setEnableWebResearch(slideSpec.webResearchEnabled);
    setRenderMode(slideSpec.renderMode);
    setSvgEffort(slideSpec.svgReasoningEffort);
  }, [slideSpec?.deckId]);
  ...
  useEffect(() => {
    (async () => {
      try {
        const preferences = await loadSlideSpecPreferences();
        if (!preferences) return;
        setDetailLevel(preferences.detailLevel);
        setGenerationMode(preferences.generationMode);
        setActiveFieldNames(preferences.activeFields);
        setImagePromptFields(preferences.imagePromptFields);
        setAvailableFields(preferences.availableFields);
      } catch { /* ignore */ }
    })();
  }, []);

  // Debounced save settings to DB
  const savePreferencesDebounced = useCallback((patch: SlideSpecPreferencesPatch) => {
    if (settingsSaveTimerRef.current) clearTimeout(settingsSaveTimerRef.current);
    settingsSaveTimerRef.current = setTimeout(async () => {
      try {
        await saveSlideSpecPreferences(patch);
      } catch (e) {
  ...
  const handleFieldDragEnd = useCallback((event: DragEndEvent) => {
    ...
    saveSlideSpecFieldCatalog(nextPool);
    // 激活字段按新池顺序重排
    const activeSet = new Set(activeFieldNames);
    const nextActive = nextPool.filter(f => activeSet.has(f));
    setActiveFieldNames(nextActive);
    savePreferencesDebounced({ activeFields: nextActive });
  }, [activeFieldNames, availableFields, savePreferencesDebounced]);
  ...
  // Source-deck import: asynchronous job polling.
  useEffect(() => {
    if (!deckId) return;
    const jobId = readSourceDeckJobId();
    if (!jobId) return;

    setIsSourceDeckProcessing(true);
    let cancelled = false;
    let pollFailCount = 0;

    const poll = async () => {
      try {
        const job = await loadSourceDeckJob(deckId, jobId);
        if (cancelled) return;
        if (!job) return;
        pollFailCount = 0; // reset on success

        setSourceDeckProgress({ total: job.total, completed: job.completed });

        await syncProject(deckId);

        if (job.state === 'completed') {
          clearSourceDeckJob();
          setIsSourceDeckProcessing(false);
          setSourceDeckProgress(null);
          await syncProject(deckId);
          return;
        }
        ...
        if (job.state === 'failed') {
          ...
          setIsSourceDeckProcessing(false);
          setSourceDeckProgress(null);
          show({ message: job.errorMessage || t('detail.renovationFailed'), type: 'error' });
          return;
        }
        ...
        if (cancelled) return;
        pollFailCount++;
        console.error('Source-deck job poll error:', err);
        if (pollFailCount >= 5) {
          clearSourceDeckJob();
          setIsSourceDeckProcessing(false);
          setSourceDeckProgress(null);
  ...
  // 加载项目数据
  useEffect(() => {
    if (deckId && (!slideSpec || slideSpec.deckId !== deckId)) {
      syncProject(deckId);
    } else if (deckId && slideSpec?.deckId === deckId && !slideSpec.hasAnySpec) {
      syncProject(deckId);
    }
  }, [deckId, slideSpec?.deckId]);

  // 同步描述生成要求
  useEffect(() => {
    if (slideSpec) {
      setSpecRequirements(slideSpec.requirements);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        await saveSlideSpecRequirements(deckId, specRequirements);
        setIsRequirementsDirty(false);
      } catch (e) {
  ...
  }, [deckId, isRequirementsDirty, specRequirements]);
  ...
  const handleGenerateAll = async () => {
    const executeGenerate = async () => {
      await generateSlideSpecs(detailLevel, enableWebResearch, renderMode, svgEffort);
    };

    if (slideSpec?.hasAnySpec) {
      confirm(
  ...
  const handleRegenerateSlide = async (slideId: string) => {
    ...
    const slide = slides.find((candidate) => slideIdentity(candidate) === slideId);
    if (!slide) return;

    const executeRegenerate = async () => {
      try {
        if (slideSpec.sourceDeckMode) {
          await regenerateSourceSlide(slideId);
        } else {
          await generateSingleSlideSpec(slideId, detailLevel, enableWebResearch);
        }
  ...
  const regenerateSlideRef = useRef(handleRegenerateSlide);
  regenerateSlideRef.current = handleRegenerateSlide;
  const stableRegenerateSlide = useCallback((slideId: string) => {
    regenerateSlideRef.current(slideId);
  }, []);

  const handleAiRefineSlideSpecs = useCallback(async (requirement: string, previousRequirements: string[]) => {
    if (!deckSnapshot || !deckId) return;

    try {
      const message = await refineSlideSpecs(deckId, requirement, previousRequirements);
      await syncProject(deckId);
      show({
        message: message || t('detail.messages.refineSuccess'),
        type: 'success'
      });
    } catch (error: any) {
  ...
  }, [deckId, deckSnapshot, syncProject, show, t]);

  // 导出页面描述为 Markdown 文件
  const handleExportSlideSpecs = useCallback(() => {
    if (!deckSnapshot) return;
    exportLegacyDeckToMarkdown(deckSnapshot, { outline: false, description: true });
  ...
  const handleExportFull = useCallback(() => {
    if (!deckSnapshot) return;
    exportLegacyDeckToMarkdown(deckSnapshot);
    show({ message: t('detail.messages.exportSuccess'), type: 'success' });
  }, [deckSnapshot, show, t]);

  // 导入描述 Markdown（追加新页面）
  const handleImportSlideSpecs = useCallback(async (text: string) => {
    if (!deckSnapshot || !deckId) return;
    try {
      const parsed = parseLegacyMarkdownSlides(text);
      if (parsed.length === 0) {
  ...
  if (!deckSnapshot || !slideSpec) {
    return <Loading fullscreen message={t('detail.messages.loadingProject')} />;
  }

  const allSpecsReady = slideSpec.isComplete;
  const missingDescCount = slideSpec.missingSlideCount;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-background-primary flex flex-col">
        ...
        <div className="flex items-center justify-between gap-2 md:gap-4">
            ...
            <Button
              ...
              onClick={() => {
                if (fromHistory) {
                  ...
                  navigate(`/project/${deckId}/outline`);
                }
              }}
              disabled={isSourceDeckProcessing}
          ...
          <div className="flex-1 max-w-xl mx-auto hidden md:block md:-translate-x-3 pr-10">
            <AiRefineInput
              ...
              onSubmit={handleAiRefineSlideSpecs}
              disabled={isSourceDeckProcessing}
          ...
          <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
            <Button
              ...
              size="sm"
              icon={<ArrowLeft size={16} className="md:w-[18px] md:h-[18px]" />}
              onClick={() => navigate(`/project/${deckId}/outline`)}
              disabled={isSourceDeckProcessing}
            ...
            <Button
              ...
              onClick={() => navigate(`/project/${deckId}/preview`)}
              disabled={!allSpecsReady || isSourceDeckProcessing}
              title={!allSpecsReady && !isSourceDeckProcessing ? t('detail.disabledNextTip', { count: missingDescCount }) : undefined}
        ...
        {isSourceDeckProcessing ? (
          <div className="max-w-xl mx-auto">
            <div className="flex items-center justify-between mb-1.5">
              ...
              {sourceDeckProgress && sourceDeckProgress.total > 0 && (
                <span className="text-sm font-medium text-brand-600 dark:text-brand">
                  {t('detail.renovationProgress', { completed: String(sourceDeckProgress.completed), total: String(sourceDeckProgress.total) })}
                </span>
              )}
            </div>
            <div className="w-full h-2.5 bg-gray-200 dark:bg-background-hover rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: sourceDeckProgress && sourceDeckProgress.total > 0
                    ? `${Math.round((sourceDeckProgress.completed / sourceDeckProgress.total) * 100)}%`
                    : '0%',
                  animation: !sourceDeckProgress || sourceDeckProgress.total === 0
                    ? 'pulse 1.5s ease-in-out infinite'
                    : undefined,
                  minWidth: !sourceDeckProgress || sourceDeckProgress.completed === 0 ? '10%' : undefined,
                }}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1">
            ...
            <div className="relative" ref={settingsRef}>
              <Button
                ...
                icon={<span className="relative"><Settings2 size={16} />{specRequirements && <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-brand-400" />}</span>}
                      ...
                      {(['streaming', 'parallel'] as const).map(mode => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            savePreferencesDebounced({ generationMode: mode });
                          }}
                        >
                          {t(`detail.${mode}`)}
                        </button>
                      ...
                      <SortableContext items={availableFields} strategy={rectSortingStrategy}>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {availableFields.map(name => {
                            const active = activeFieldNames.includes(name);
                            return (
                              <SortableFieldPill
                                key={name}
                                name={name}
                                active={active}
                                removable={!DEFAULT_SLIDE_SPEC_FIELDS.has(name)}
                                onToggle={() => {
                                  const next = active
                                    ? activeFieldNames.filter(f => f !== name)
                                    : [...activeFieldNames, name];
                                  setActiveFieldNames(next);
                                  savePreferencesDebounced({ activeFields: next.length > 0 ? next : defaultSlideSpecFields() });
                                }}
                                ...
                                onToggleImagePrompt={() => {
                                  ...
                                  savePreferencesDebounced({ imagePromptFields: next });
                                }}
                                onRemove={() => {
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter' && newFieldName.trim()) {
                            ...
                            if (!availableFields.includes(trimmed) && availableFields.length < 10) {
                              ...
                              saveSlideSpecFieldCatalog(nextPool);
                              // 新增字段默认勾选
                              const nextActive = [...activeFieldNames, trimmed];
                              setActiveFieldNames(nextActive);
                              savePreferencesDebounced({ activeFields: nextActive });
                              setNewFieldName('');
                            }
                  ...
                  <div className="border-t border-gray-100 dark:border-border-primary pt-3">
                    ...
                    <div data-testid="desc-requirements-textarea">
                      <MarkdownTextarea
                        ...
                        onChange={(val) => { setSpecRequirements(val); setIsRequirementsDirty(true); }}
                    ...
                    <PresetCapsules
                      type="description"
                      onAppend={(text) => {
                        setSpecRequirements((prev) => prev ? `${prev}\n${text}` : text);
                        setIsRequirementsDirty(true);
                      }}
                    />
                  </div>
            ...
            <div className="relative" ref={fileMenuRef}>
              ...
              {fileMenuOpen && (
                <div className="absolute top-full right-0 mt-1 z-50 min-w-[160px] rounded-lg border border-gray-200 dark:border-border-primary bg-white dark:bg-background-secondary shadow-lg dark:shadow-none overflow-hidden">
                  <button
                    type="button"
                    onClick={() => { handleExportSlideSpecs(); setFileMenuOpen(false); }}
                    disabled={!slideSpec.hasAnySpec}
            ...
            <span className="text-xs md:text-sm text-gray-500 dark:text-foreground-tertiary whitespace-nowrap">
              {slideSpec.completedSlideCount} /{' '}
              {slides.length} {t('detail.pagesCompleted')}
            </span>
          </div>
        </div>
        )}
      ...
      <main className="flex-1 p-3 md:p-6 overflow-y-auto min-h-0">
        <div className="max-w-7xl mx-auto">
          <ReferenceFileList
            ...
            showToast={show}
          />
          {slides.length === 0 && !isSourceDeckProcessing ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
              {isSourceDeckProcessing && slides.length === 0 ? (
                /* Placeholder cards while the source-deck job creates slides. */
                Array.from({ length: sourceDeckProgress?.total || 6 }).map((_, index) => (
                  <LegacySlideSpecCard
                    key={`skeleton-${index}`}
                    page={{ id: `skeleton-${index}`, title: '', sort_order: index, status: 'GENERATING_DESCRIPTION' } as any}
                    index={index}
                    projectId={slideSpec.deckId}
                    extraFieldNames={activeFieldNames}
                ...
                const slideId = slideIdentity(slide);
                const visibleSlide = markSlideSpecGenerating(slide, isSourceDeckProcessing);
                return (
                  <LegacySlideSpecCard
                    ...
                    imagePromptFields={imagePromptFields}
                    showToast={show}
                    onUpdate={(data) => { if (slideId) updateSlideLocal(slideId, data); }}
                    onRegenerate={() => { if (slideId) stableRegenerateSlide(slideId); }}
      ...
      <ImportMarkdownModal
        ...
        onImport={handleImportSlideSpecs}
