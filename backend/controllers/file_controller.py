...
# backend/fonts — the SVG whitelist font dir. Served to the frontend so inline
# <svg> previews render CJK text with the SAME font (Noto Sans CJK SC) that resvg
# uses for the PNG and that the editable-PPTX export uses; otherwise the browser
# substitutes a different font and the preview diverges from the real output.
_FONTS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'fonts')


@file_bp.route('/fonts/<filename>', methods=['GET'])
def serve_font(filename):
    """GET /files/fonts/{filename} - Serve a whitelist font for frontend @font-face.

    Only well-known font extensions are allowed; the file is immutable so we cache
    hard (the TTF is large — ~16MB — and never changes).
    """
    try:
        safe = secure_filename(filename)
        if not safe.lower().endswith(('.ttf', '.otf', '.woff', '.woff2')):
            return not_found('Font')
        if not os.path.exists(os.path.join(_FONTS_DIR, safe)):
            return not_found('Font')
        resp = send_from_directory(_FONTS_DIR, safe)
