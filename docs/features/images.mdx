...
Configure export error handling:
- **Allow Partial Results**: continue where possible and return partial output if some pages fail
- Editable PPTX export is only available for SVG-route projects; image-route OCR/MinerU/inpainting reverse engineering has been removed
