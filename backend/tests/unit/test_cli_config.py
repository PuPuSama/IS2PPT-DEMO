...
from cli.is2ppt_cli.config import default_config_path, resolve_config
from cli.is2ppt_cli.identity import CONFIG_DIR_NAME, ENV_PREFIX
...
def test_resolve_config_reads_default_is2ppt_config(tmp_path: Path, monkeypatch):
    _clear_cli_env(monkeypatch)
    monkeypatch.setenv("APPDATA", str(tmp_path / "appdata"))
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    ...
    assert cfg.access_code == "current"
