...
def _build_research_topic(project: Project, outline: list = None) -> str:
    """Build a concise research topic from project metadata and outline pages."""
    parts = []
    for value in (
    ...
    if outline:
        ...
        for item in outline:
            if isinstance(item, dict) and item.get('pages'):
                if item.get('part'):
                    outline_lines.append(str(item.get('part')))
                for page in item.get('pages') or []:
                    if not isinstance(page, dict):
                        continue
                    title = page.get('title') or ''
                    points = page.get('points') or []
                    outline_lines.append(f"{title}: {'; '.join(points)}".strip(': '))
            elif isinstance(item, dict):
                title = item.get('title') or ''
                points = item.get('points') or []
                outline_lines.append(f"{title}: {'; '.join(points)}".strip(': '))
        outline_text = "\n".join(line for line in outline_lines if line)
        if outline_text:
            parts.append(f"当前 PPT 大纲：\n{outline_text}")

    return "\n\n".join(parts)[:3000]


def _maybe_run_web_research_for_project(project: Project, data: dict, ai_service, language: str, topic: str = None):
    """Run optional web research and persist the result on the project."""
    enable_research = data.get('enable_web_research')
    if enable_research is None:
        enable_research = bool(getattr(project, 'enable_web_research', False))
    else:
        project.enable_web_research = bool(enable_research)

    if not enable_research:
        return None

    research_topic = (topic or _build_research_topic(project)).strip()
    if not research_topic:
        logger.warning(f"联网调研跳过: 项目 {project.id} 缺少可检索主题")
        return None

    try:
        from services import research_service
        research = research_service.run_research(
            research_topic, ai_service=ai_service, language=language
        )
        project.research_context = research.get('research_context') or None
        project.set_research_sources(research.get('sources') or [])
        project.updated_at = datetime.utcnow()
        logger.info(
            f"联网调研完成: 项目 {project.id}, {len(research.get('sources') or [])} 个来源"
        )
        return research
    except Exception as research_err:
        logger.warning(f"联网调研失败，跳过继续生成: {research_err}")
        return None
...
def generate_outline(project_id):
    ...
    try:
        ...
        if project.creation_type == 'outline':
            ...
            # 可选：联网调研（按项目开关，可被请求体 enable_web_research 覆盖）
            enable_research = data.get('enable_web_research')
            if enable_research is None:
                enable_research = bool(getattr(project, 'enable_web_research', False))
            else:
            ...
            if enable_research:
                try:
                    from services import research_service
                    research = research_service.run_research(
                        idea_prompt, ai_service=ai_service, language=language
                    )
                    project.research_context = research.get('research_context') or None
                    project.set_research_sources(research.get('sources') or [])
                    logger.info(
                        f"联网调研完成: 项目 {project_id}, {len(research.get('sources') or [])} 个来源"
                    )
                except Exception as research_err:
                    logger.warning(f"联网调研失败，跳过继续生成: {research_err}")
        ...
        with app.app_context():
            try:
                ...
                if proj.creation_type not in ('outline', 'descriptions'):
                    ...
                    enable_research = data.get('enable_web_research')
                    if enable_research is None:
                        enable_research = bool(getattr(proj, 'enable_web_research', False))
                    else:
                        proj.enable_web_research = bool(enable_research)
                    if enable_research:
                        yield _sse_event('status', {'phase': 'researching'})
                        try:
                            from services import research_service
                            research = research_service.run_research(
                                idea_prompt, ai_service=ai_service, language=language
                            )
                            proj.research_context = research.get('research_context') or None
                            proj.set_research_sources(research.get('sources') or [])
                            db.session.commit()
                            yield _sse_event('status', {
                                'phase': 'research_done',
                                'sources': research.get('sources') or [],
                            })
                            logger.info(f"流式-联网调研完成: {len(research.get('sources') or [])} 个来源")
                        except Exception as research_err:
                            logger.warning(f"流式-联网调研失败，跳过: {research_err}")
                            yield _sse_event('status', {'phase': 'research_failed'})
...
def generate_from_description(project_id):
    ...
    try:
        ...
        _maybe_run_web_research_for_project(
            project,
            data,
            ai_service,
            language,
            topic=_build_research_topic(project),
        )
...
def generate_descriptions(project_id):
    ...
    try:
        ...
        # 页面生成方式在“生成描述”这一步选定，存到项目级，并决定走哪种描述 prompt
        generation_mode = (data.get('generation_mode') or project.generation_mode or 'image').lower()
        if generation_mode not in ('image', 'svg'):
            generation_mode = 'image'
        prev_mode = (project.generation_mode or 'image').lower()
        project.generation_mode = generation_mode
        # 切换生成路线（image↔svg）时，旧路线产出的“当前页面”已不适用（SVG 的便当版面
        # 不用于生图，反之亦然）。把每页**当前指针**清空，预览回到“待在新路线下重生成”，
        # 不再残留旧路线的图/SVG；描述随后由本任务按新路线整体重生成。
        # **保留 PageImageVersion 历史记录与文件**——前端按 page.id 加载版本、与当前指针
        # 无关，用户可在版本菜单切回旧版本对比新旧路线效果。
        if prev_mode != generation_mode:
            for _pg in pages:
                _pg.generated_image_path = None
                _pg.generated_svg_path = None
                _pg.cached_image_path = None
        # SVG 生成的 reasoning effort（项目级，前端可选）；非法值忽略，仅在显式提供时更新
        svg_effort = data.get('svg_reasoning_effort')
        if svg_effort is not None:
            svg_effort = str(svg_effort).lower()
            if svg_effort in ('low', 'medium', 'high', 'xhigh'):
                project.svg_reasoning_effort = svg_effort
        ...
        _maybe_run_web_research_for_project(
            ...
            ai_service,
            language,
            topic=_build_research_topic(project, outline),
        )
...
def generate_descriptions_stream(project_id):
    ...
    # 生成路线（image/svg）在“生成描述”这步选定、存项目级，决定走哪种流式描述 prompt。
    # 流式现已支持 SVG“施工图”描述（与逐页版同源 prompt + 同一流式解析器）。
    generation_mode = (data.get('generation_mode') or project.generation_mode or 'image').lower()
    if generation_mode not in ('image', 'svg'):
        generation_mode = 'image'
    prev_mode = (project.generation_mode or 'image').lower()
    project.generation_mode = generation_mode
    # 切换路线(image↔svg)时清空每页当前指针（保留 PageImageVersion 历史），与并行 endpoint 一致。
    if prev_mode != generation_mode:
        for _pg in Page.query.filter_by(project_id=project_id).all():
            _pg.generated_image_path = None
            _pg.generated_svg_path = None
            _pg.cached_image_path = None
    # SVG 生成的 reasoning effort（项目级，前端可选）；仅在显式提供且合法时更新
    svg_effort = data.get('svg_reasoning_effort')
    if svg_effort is not None:
        svg_effort = str(svg_effort).lower()
        if svg_effort in ('low', 'medium', 'high', 'xhigh'):
    ...
    def sse_generate():
        with app.app_context():
            try:
                ...
                if enable_research:
                    yield _sse_event('status', {'phase': 'researching'})
                    research = _maybe_run_web_research_for_project(
                        proj,
                        data,
                        ai_service,
                        language,
                        topic=_build_research_topic(proj, outline),
                    )
                    db.session.commit()
                    yield _sse_event('status', {
                        'phase': 'research_done' if research else 'research_failed',
