...
class Page(db.Model):
    ...
    generated_svg_path = db.Column(db.String(500), nullable=True)  # Source SVG path (svg generation mode)
    ...
    def to_dict(self, include_versions=False):
        ...
        generated_svg_url = None
        if self.generated_svg_path:
            svg_filename = Path(self.generated_svg_path).name
            generated_svg_url = f'/files/{self.project_id}/pages/{svg_filename}'
        ...
        data = {
            ...
            'generated_svg_url': generated_svg_url,
            'status': self.status,
