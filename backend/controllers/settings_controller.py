...
def update_settings():
    ...
    try:
        ...
        # 页面生成模式: image (位图) / svg (整页 SVG)
        if "generation_mode" in data:
            gen_mode = data["generation_mode"]
            if gen_mode not in ("image", "svg"):
                return bad_request("generation_mode must be 'image' or 'svg'")
            settings.generation_mode = gen_mode
