...
@page_bp.route('/<project_id>/pages/<page_id>/svg', methods=['GET', 'PUT'])
def page_svg(project_id, page_id):
    """
    GET  /api/projects/{pid}/pages/{page_id}/svg  -> {"svg": "<svg ...>"}
    PUT  /api/projects/{pid}/pages/{page_id}/svg  body {"svg": "<svg ...>"}
    ...
    if request.method == 'GET':
        if not page.generated_svg_path:
            return not_found('SVG (this page was not generated in SVG mode)')
        abs_path = file_service.get_absolute_path(page.generated_svg_path)
        if not Path(abs_path).exists():
            return not_found('SVG file')
        with open(abs_path, 'r', encoding='utf-8') as f:
            return success_response({'svg': f.read()})

    # PUT — save edited SVG
    from services.svg_contract import clean_svg, validate_svg
    from services.svg_render_service import render_svg_to_png
    from services.task_manager import save_image_with_version, save_page_svg

    data = request.get_json() or {}
    svg_in = data.get('svg')
    if not svg_in or '<svg' not in svg_in:
        return bad_request('Request body must include an "svg" string')

    svg = clean_svg(svg_in)
    ok, err = validate_svg(svg)
    if not ok:
        return bad_request(f'SVG 不合规: {err}')

    try:
        image = render_svg_to_png(svg, width=2560)
    except Exception as e:
        logger.warning(f"Edited SVG failed to render for page {page_id}: {e}")
        return bad_request(f'SVG 渲染失败: {e}')

    try:
        image_path, _version = save_image_with_version(
            image, project_id, page_id, file_service, page_obj=page
        )
        save_page_svg(svg, image_path, file_service, page_obj=page)
        page.updated_at = datetime.utcnow()
        if page.project:
            page.project.updated_at = datetime.utcnow()
        db.session.commit()
        ...
        db.session.rollback()
        logger.error(f"Failed to save edited SVG for page {page_id}: {e}")
        return error_response('SERVER_ERROR', 'An internal server error occurred', 500)
...
def set_current_image_version(project_id, page_id, version_id):
    ...
    try:
        ...
        # 前端内联显示的是 generated_svg_path，切换版本时它必须**跟随目标版本**：
        # SVG 模式版本的 PNG 旁存了同名 .svg（save_page_svg）→ 指向它（内联矢量）；
        # 生图模式版本没有 .svg 兄弟 → 清空（显示该版本 PNG）。否则在同一页混有 SVG /
        # 生图两种版本时，切到生图版本仍会残留旧 SVG。
        svg_sibling = version.image_path.rsplit('.', 1)[0] + '.svg'
        page.generated_svg_path = svg_sibling if file_service.file_exists(svg_sibling) else None
