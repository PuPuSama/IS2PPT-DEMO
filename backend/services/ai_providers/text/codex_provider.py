...
class CodexTextProvider(TextProvider):
    ...
    def _build_payload(self, prompt: str, reasoning_effort: str = None) -> dict:
        """Build a Responses API request body. Stream is always true (required by Codex).

        ``reasoning_effort`` (e.g. 'low'/'medium'/'high'/'xhigh') maps to the
        Responses API ``reasoning.effort`` knob; omitted -> model default.
        """
        payload = {
            "model": self.model,
    ...
    def generate_text(self, prompt: str, thinking_budget: int = 0,
        ...
        ``reasoning_effort`` lets callers request a higher reasoning budget per call
        (e.g. 'xhigh' for SVG design); ignored by providers that don't support it.
        """
        resp = self._post_with_retry(self._build_payload(prompt, reasoning_effort=reasoning_effort))
        collected = []
