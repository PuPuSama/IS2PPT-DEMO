...
const debouncedUpdatePage = debounce(
  ...
  generateDescriptions: async (detailLevel?: string, enableWebResearch?: boolean, pageGenMode: 'image' | 'svg' = 'image', svgReasoningEffort?: SvgReasoningEffort) => {
    ...
    if (mode === 'streaming') {
      ...
      try {
        ...
        const response = await api.generateDescriptions(projectId, undefined, detailLevel, enableWebResearch, pageGenMode, svgReasoningEffort);
        const taskId = response.data?.task_id;
  ...
  generatePageDescription: async (pageId: string, detailLevel?: string, enableWebResearch?: boolean) => {
    ...
    try {
      const response = await api.generatePageDescription(currentProject.id!, pageId, true, undefined, detailLevel, enableWebResearch);
