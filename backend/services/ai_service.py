...
from .prompts import (
    ...
    get_svg_page_description_prompt,
    SVG_DESC_EXTRA_FIELDS,
    get_all_descriptions_stream_prompt,
    get_svg_all_descriptions_stream_prompt,
    get_image_generation_prompt,
    get_svg_generation_prompt,
...
# SVG 整页设计对推理敏感：reasoning effort 由项目级 svg_reasoning_effort 选定（前端可选）。
# gpt-5.5 /responses 支持的档位——注意 'minimal' 被 gpt-5.5 明确拒绝(400)，故不在内。
# effort 越高排版/结构越好但越慢(整页 SVG：high 适中、xhigh 实测 ~327s/页)；
# 不支持该参数的 provider 由 generate_svg 优雅回退(无 effort)。
VALID_SVG_REASONING_EFFORTS = ('low', 'medium', 'high', 'xhigh')
DEFAULT_SVG_REASONING_EFFORT = 'high'
...
class ProjectContext:
    ...
    def __init__(self, project_or_dict, reference_files_content: Optional[List[Dict[str, str]]] = None):
        ...
        if hasattr(project_or_dict, 'idea_prompt'):
            ...
            self.research_context = getattr(project_or_dict, 'research_context', None)
        else:
            # 是字典
            ...
            self.research_context = project_or_dict.get('research_context')
...
class AIService:
    ...
    def generate_search_queries(self, topic: str, max_queries: int = 4, language: str = None) -> List[str]:
        """Generate a few diverse web-search queries for a topic (uses the text model)."""
        prompt = get_search_queries_prompt(topic, max_queries=max_queries, language=language)
        text = self.text_provider.generate_text(prompt, thinking_budget=0) or ""
        seen, queries = set(), []
    ...
    def summarize_research(self, topic: str, results, language: str = None) -> str:
        """Distill raw search results (objects with title/url/content) into a briefing."""
        blocks = []
        ...
        results_text = "\n\n".join(blocks)
        prompt = get_research_summary_prompt(topic, results_text, language=language)
        return (self.text_provider.generate_text(prompt, thinking_budget=0) or "").strip()
    ...
    def generate_page_description(self, project_context: ProjectContext, outline: List[Dict],
                                 ...
                                 generation_mode: str = 'image') -> Dict:
        """
        ...
        Args:
            ...
            generation_mode: 'image'（位图描述，含视觉元素/视觉焦点/排版布局）或
                'svg'（“施工图”描述：页面类型/主旨/要点数据/便当版面/视觉层级/占位元素 + 演讲者备注）
        ...
        # SVG 路线：用“施工图”描述 prompt（ppt-agent 03-planning），只把“演讲者备注”当字段抽出，
        # 其余规划正文作为 page_desc 喂给 get_svg_generation_prompt。
        if (generation_mode or 'image').lower() == 'svg':
            extra_field_names = list(SVG_DESC_EXTRA_FIELDS)
            desc_prompt = get_svg_page_description_prompt(
    ...
    def generate_descriptions_stream(self, project_context: ProjectContext,
        ...
        generation_mode='svg' 走“施工图”流式 prompt + SVG 字段集（与逐页版同源），
        其余走原位图风描述。下游的逐行流式解析器(状态机)对两者通用、无需分支。
        """
        if (generation_mode or 'image').lower() == 'svg':
            extra_field_names = list(SVG_DESC_EXTRA_FIELDS)
            prompt = get_svg_all_descriptions_stream_prompt(
                project_context=project_context,
                outline=outline,
    ...
    def generate_svg(self, outline: List[Dict], page: Dict, page_desc: str,
                     page_index: int, has_material_images: bool = False,
                     extra_requirements: Optional[str] = None, language: str = 'zh',
                     has_template: bool = True, aspect_ratio: str = "16:9",
                     reasoning_effort: Optional[str] = None,
                     max_attempts: int = 3) -> str:
        """Generate a full-page SVG (SVG generation mode) via the text provider.

        Mirrors generate_image_prompt's inputs so task_manager can branch on
        generation_mode. Builds the contract prompt, calls the text model, then
        extracts/cleans/validates the SVG; on a contract violation it retries with a
        correction hint up to ``max_attempts`` times.

        Returns:
            A contract-valid SVG string (font-family normalised to the whitelist).

        Raises:
            ValueError: if no valid SVG is produced after ``max_attempts`` tries
                (caller may fall back to image mode or mark the page failed).
        """
        from .svg_contract import extract_clean_validate

        # Resolve reasoning effort (project-level, frontend-selectable); invalid/None -> default.
        effort = (reasoning_effort or DEFAULT_SVG_REASONING_EFFORT).lower()
        if effort not in VALID_SVG_REASONING_EFFORTS:
            effort = DEFAULT_SVG_REASONING_EFFORT

        outline_text = self.generate_outline_text(outline)
        current_section = page['part'] if 'part' in page else f"{page.get('title', 'Untitled')}"
        cleaned_desc = self.remove_markdown_images(page_desc)

        base_prompt = get_svg_generation_prompt(
            page_desc=cleaned_desc,
            outline_text=outline_text,
            current_section=current_section,
            has_material_images=has_material_images,
            extra_requirements=extra_requirements,
        ...
        for attempt in range(1, max_attempts + 1):
            logger.info(f"🎨 generate_svg attempt {attempt}/{max_attempts} for page {page_index} (reasoning={effort})")
            # SVG 设计吃推理：按所选 effort 请求 reasoning（gpt-5.x /responses 支持 low/medium/high/xhigh）；
            # 不支持该参数的 provider 优雅回退（无 effort）。
            try:
                response = self.text_provider.generate_text(prompt, reasoning_effort=effort)
            except TypeError:
                response = self.text_provider.generate_text(prompt, thinking_budget=0)
            svg, err = extract_clean_validate(response or "")
            if svg:
                logger.debug(f"generate_svg ok on attempt {attempt} ({len(svg)} chars)")
                return svg
            last_err = err
            logger.warning(f"generate_svg invalid on attempt {attempt}: {err}")
            prompt = base_prompt + (
                f"\n\n## 上次输出不合规\n原因：{err}\n"
                f"请修正后**只重新输出完整的 <svg>…</svg>**，不要解释、不要 markdown 围栏。"
            )

        raise ValueError(f"generate_svg failed after {max_attempts} attempts: {last_err}")
