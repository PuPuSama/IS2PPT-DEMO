...
def create_app():
    ...
    with app.app_context():
        ...
        # Reap zombie async tasks left over from a previously killed backend run
        _recover_zombie_tasks(app)
...
def _recover_zombie_tasks(app):
    """Reap zombie async tasks left over from a previously killed backend run.

    When the server is stopped/restarted while tasks are running, their
    in-memory workers die but the DB rows stay in a non-terminal state
    (PENDING/PROCESSING), so the UI shows them spinning forever. On startup we
    mark those tasks FAILED and revert any page left in a transient generating
    state back to its last stable status (see services/task_manager.py for the
    matching per-stage failure conventions).
    """
    from datetime import datetime
    from models import Task, Page

    # Transient page status -> recovered status. Image-stage states have an
    # unambiguous pre-generation state (DESCRIPTION_GENERATED); the description
    # stage mirrors the app's own description-failure convention (FAILED).
    PAGE_RECOVERY = {
        'GENERATING': 'DESCRIPTION_GENERATED',
        'QUEUED': 'DESCRIPTION_GENERATED',
        'GENERATING_DESCRIPTION': 'FAILED',
    }

    try:
        zombies = Task.query.filter(Task.status.notin_(['COMPLETED', 'FAILED'])).all()

        pages_reset = 0
        for src, dst in PAGE_RECOVERY.items():
            pages_reset += Page.query.filter(Page.status == src).update(
                {'status': dst}, synchronize_session=False
            )

        now = datetime.utcnow()
        note = '[startup recovery] interrupted by backend restart'
        for task in zombies:
            task.status = 'FAILED'
            task.error_message = f"{task.error_message}\n{note}" if task.error_message else note
            task.completed_at = now

        if zombies or pages_reset:
            db.session.commit()
            logging.warning(
                f"Startup recovery: reaped {len(zombies)} zombie task(s), "
                f"reset {pages_reset} stuck page(s)"
            )
        else:
            db.session.rollback()
            logging.info("Startup recovery: no zombie tasks found")
    except Exception as e:
        db.session.rollback()
        if isinstance(e, SQLAlchemyError) and "no such table" in str(e):
            logging.debug(f"Startup recovery skipped (tables not ready): {e}")
        else:
            logging.warning(f"Startup recovery failed: {e}")
...
if __name__ == '__main__':
    ...
    # NOTE: use_reloader is forced off. On this Windows + uv/venv setup the Werkzeug
    # auto-reloader fails to hand off WERKZEUG_RUN_MAIN between the .venv and uv
    # interpreters, recursively spawning nested app.py processes while a wedged child
    # keeps holding port 5011 (backend appears "hung"). Restart manually to pick up
    # code changes. Override with BACKEND_RELOAD=1 if you really want it.
